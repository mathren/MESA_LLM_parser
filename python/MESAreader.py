...
# Copyright (C) 2019-2023 Mathieu Renzo
...
try:
    ...
    secyer = const_def.secyer.value*u.s/u.yr
    G_cgs = const_def.standard_cgrav.value*u.cm**3/(u.g*u.s**2)
    Lsun = const_def.Lsun.value*u.erg/u.s
    Msun = const_def.Msun.value*u.g
    Rsun_cm = const_def.Rsun.value*u.cm
    clight = const_def.clight.value*u.cm/u.s
    print(colored("successfully read constants from pyMesa", "blue"))
except:
    try:
        """ import from astropy """
        from astropy.constants import M_sun, R_sun, L_sun, G, c
        import astropy.units as u
        Msun = M_sun.to(u.g)
        Rsun_cm = R_sun.to(u.cm)
        Lsun = L_sun.to(u.erg/u.s)
        G_cgs = G.to(u.cm**3/(u.g*u.s**2))
        clight = c.to(u.cm/u.s)
        print(colored("imported constants from astropy", "blue"))
    except:
